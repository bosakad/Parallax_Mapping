...
void Camera::MouseLook(int mouseX, int mouseY){
    ...
    glm::vec2 mouseDir = newMousePosition - m_oldMousePosition;

    // Rotate about the upVector

    float RotationSpeed = 0.1f;
    float angle = -glm::radians(mouseDir[0] * RotationSpeed);

    m_viewDirection = glm::rotate( m_viewDirection, angle, glm::vec3(0, 1, 0) );
...
void Camera::MoveLeft(float speed){
    ...
    glm::vec3 leftDir = -glm::cross(m_viewDirection, m_upVector);

    m_eyePosition += leftDir * speed;

}

void Camera::MoveRight(float speed){
    
    glm::vec3 rightDir = glm::cross(m_viewDirection, m_upVector);
